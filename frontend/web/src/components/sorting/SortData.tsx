...
interface SortDataProps<TData, TValue>
    extends React.HTMLAttributes<HTMLDivElement> {
    column: Column<TData, TValue>
    title: string
}

const SortData = <TData, TValue>({
    column,
    title,
    className,
}: SortDataProps<TData, TValue>) => {
    if (!column.getCanSort()) {
        return <div className={cn(className)}>{title}</div>
    }

    return (
        <div className={cn("w-full flex justify-center items-center space-x-2 m-auto", className)}>
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <Button
                        variant="ghost"
                        size="sm"
                        className="w-full -ml-3 h-8 data-[state=open]:bg-accent"
                    >
                        <span>{title}</span>
